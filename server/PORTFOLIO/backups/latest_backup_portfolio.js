...
function update_portfolio_file(fileName){
    let data = JSON.stringify(portfolio_data);
    fs.writeFile('./LOCAL_DB/test.json', data, (err) => {
        if(err){
...
function getGridTemplate(data, sectionName, sectionIndex, partsIndex, application){
  ...
  data.forEach((ele, index) => {
    ...
    if(ele.event !== undefined && ele.event !== '' && Array.isArray(ele.event)){
        ...
        for (var i = 0; i < ele.event.length; i++) {
            if(ele.event[i].argument !== undefined && ele.event[i].argument !== ''){
              ...
              if(sectionName === 'shefsRestaurant'){
                console.log("&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&");
    ...
    if(ele.type === 'text'){
      ...
      if(ele.key.name && ele.key.type === "font-awesome"){
          ...
          subTemplate += '<div class="icon" id="'+inputId+'"><i class="icon fa fa-'+ele.key.name+' '+themeClasses+' '+classTemplate+' '+elementClass+'" style="'+ele.key.style+'" aria-hidden="true"></i></div>';
      }
      ...
      subTemplate += getColorPallateTemplate(ele, application);
        ...
        subTemplate += '<'+ele.key.name+' id="'+ele.key.id+'"  style=" '+ele.key.style+'">';
...
function sections_template(data, editable, sectionName, sectionIndex, partsIndex, application){
  let template = '';
  if(editable){
    template += getGridEditTemplate(data, sectionName, sectionIndex, partsIndex, application);
  }else{
    template += getGridTemplate(data, sectionName, sectionIndex, partsIndex, application);
  }
  return template;
}
...
function get_section_template(sectionName, editable, data, application){
        ...
        data.block.sections.forEach((section, sectionIndex) => {
            ...
            section.parts.forEach((ele, partsIndex) => {
                  ...
                  if(ele.desc !== undefined && ele.desc !== null && ele.desc && ele.desc.length > 0){
                      partsTemplate += sections_template(ele.desc, editable, sectionName, sectionIndex, partsIndex, application);
                  }
