...
function get_section_data(request){
console.log("&&&&&&&&&& get-section_data : request : ", request)
let sectionData = '';
let APP_DATA = '';
...
//   return APP_DATA[sectionName];
    if(request?.sectionName && request?.fileName){
            if(request?.fileName === 'default'){
                APP_DATA = require(ROOT_DIR+'/server/PORTFOLIO/store/generic_portfolio_data.js').sections_data;
                // console.log("==== APP_DATA :: ", APP_DATA)
                // return APP_DATA[request.sectionName];
            }else{
                APP_DATA = require(ROOT_DIR+'/server/'+request.application+'/store/'+request.fileName+'.js').sections_data;
            }
    }
    return APP_DATA[request.sectionName];
}
